...
剥离了旧版的风控验真、邮件拉取、黑名单等复杂功能，保留核心流程。
...
1. 在 `engines/` 下创建 `my_engine.py`，继承 `BaseEngine`
2. 实现 `extract(self, file_path: str) -> EngineResult`
3. 在 `processor.py` 的 `_init_engines` 中注册
...
class MyEngine(BaseEngine):
    ...
    def extract(self, file_path):
        # 识别逻辑
        return EngineResult(data=fields, confidence=0.9, engine=self.name)
```

### 新增数据库字段

1. 修改 `database.py` 中的 `init_db` 表结构和 `insert_invoice` 列列表
2. 修改 `processor.py` 的 `_build_record` 方法，增加字段映射
3. 如需前端展示，修改 `app.py` 中表格列定义和编辑表单
